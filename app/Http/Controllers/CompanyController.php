...
class CompanyController extends Controller
{
    ...
    public function companiesup(Request $request)
    {
        ...
        if (Session::has('loginId')) {
            ...
            return response()
                ->view('students.companiesup', compact('companies', 'students', 'user', 'stu', 'ojt'))
                ->withHeaders([
...
public function companyCreate(Request $request)
{
    ...
    $com->companyLink = $request->companyLink;
...
public function pending()
{
    ...
    if (Session::has('loginId')) {
        ...
        return response()
            ->view('students.pending', compact('user', 'ojt'))
            ->withHeaders([
                'Cache-Control' => 'no-cache, no-store, must-revalidate', // HTTP 1.1.
                'Pragma' => 'no-cache', // HTTP 1.0.
                'Expires' => '0', // Proxies.
...
{
    Log::info('Attempting to remove company with ID: ' . $id);

    DB::beginTransaction();

    try {
        Voucher::where('company_id', $id)->delete();
        Company::where('id', $id)->delete();

        DB::commit();

        Log::info('Company and related vouchers removed successfully.');

        return response()->json(['success' => 'Company and related vouchers removed successfully.']);
    } catch (\Exception $e) {
        DB::rollback();

        Log::error('Failed to remove company: ' . $e->getMessage());

        return response()->json(['error' => 'Failed to remove company: ' . $e->getMessage()], 500);
    }
}

public function apply($companyId)
{
    $company = Company::find($companyId);
